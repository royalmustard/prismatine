...
pub fn create_parameter_box(&self
    ...
    match self.params.prismatine_params.process_mode.value() {
        ...
        ProcessMode::KO2 => column![
            text("phase gain").width(Length::Fill).center(),
            container(
                nih_plug_iced::widgets::ParamSlider::new(
                    self.phase_gain_slider_state.clone(),
                    &self.params.prismatine_params.ko2_params.phase_gain,
                )
                .map(Message::ParamUpdate),
            )
            .width(Length::Fill)
            ...
            container(
                nih_plug_iced::widgets::ParamSlider::new(
                    self.temperature_slider_state.clone(),
                    &self.params.prismatine_params.ko2_params.temperature,
                )
                .map(Message::ParamUpdate),
            )
            .width(Length::Fill)
            ...
            container(
                nih_plug_iced::widgets::ParamSlider::new(
                    self.I_c_slider_state.clone(),
                    &self.params.prismatine_params.ko2_params.critical_current,
                )
                .map(Message::ParamUpdate),
            )
            .width(Length::Fill)
